...
static int nl80211_get_iface_frequency_cb(struct nl_msg *msg, void *arg)
{
	int *freq = arg;
	struct nlattr **attr = nl80211_parse(msg);

	*freq = attr[NL80211_ATTR_WIPHY_FREQ] ? nla_get_u32(attr[NL80211_ATTR_WIPHY_FREQ]) : 0;
...
int nl80211_get_frequency(const char *ifname, int *buf)
{
	...
	*buf = 0;
	...
	if (*buf == 0) {
		...
		if (req) {
			nl80211_send(req, nl80211_get_iface_frequency_cb, buf);
			nl80211_free(req);
		}
	}
	...
	if ((res = nl80211_wpactl_info(ifname, "SCAN", "CTRL-EVENT-SCAN-RESULTS")))
	{
		if ((res = nl80211_wpactl_info(ifname, "SCAN_RESULTS", NULL)))
		{
			              ...
			              bssid, &freq, &rssi, cipher, ssid) > 0)
			{
				...
				if (rssi >= 0)
				{
					/* when driver using NL80211_BSS_SIGNAL_UNSPEC */
					rssi = rssi * 7 / 10;
					e->signal = rssi - 110;
				}
				else
...
int nl80211_get_wps(const char *ifname, const char *op, char *buf)
{
	char *wifidev;
	char hostapd_cmd[128];
	FILE *pipe_rst;
	const char* arg = "";
	...
	if( !(wifidev = nl80211_ifname2phy(ifname)) )
	{
		return -1;
	}
	...
	if (!strcmp(op, "status")) {
		op = "wps_get_status";
	} else if (!strcmp(op, "pin_lock")) {
		op = "pin_lock_status";
	} else if (!strcmp(op, "cancel")) {
		op = "wps_cancel";
	} else if (!strcmp(op, "config")) {
		op = "get_config";
	} else if (!strcmp(op, "pbc")) {
		op = "wps_pbc";
	} else if (!strcmp(op, "pin")) {
		timeout = "120";
		op = "wps_pin any";
		arg = buf;
		checkpin = 1;
		failstring = "PIN Status: Invalid";
	} else if (!strcmp(op, "ap_pin")) {
	...
	if (checkpin) {
		...
		if (len != 4 && len != 8) {
			...
			while (--len) {
			accum += (1 + 2 * (len & 1)) * (arg[len - 1] - '0');
		}
		if (((10 - accum % 10) % 10) != (arg[7] - '0'))
			checkpin = 0;
		}
	...
	sprintf(hostapd_cmd, "/usr/sbin/hostapd_cli -p /var/run/hostapd-%s -i %s %s %s %s", wifidev, ifname, op, arg, timeout);

	if (NULL == (pipe_rst = popen(hostapd_cmd, "r")))
	{
		return -1;
	}

	memset(buf, 0, IWINFO_BUFSIZE);
	fread(buf, sizeof(char), IWINFO_BUFSIZE, pipe_rst);
	pclose(pipe_rst);
