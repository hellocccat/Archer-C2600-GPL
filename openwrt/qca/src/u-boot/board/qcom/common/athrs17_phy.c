...
unsigned short athrs17_phy_reg_read(unsigned char phy_addr, unsigned char reg_addr)
{
	const char	*devname;
	...
	if (miiphy_read (devname, phy_addr, reg_addr, &data) != 0) {
		printf("Error reading from the PHY addr=%02x reg=%02x\n",phy_addr, reg_addr);
		data = 0;
	} else {
		printf("addr=%02x reg=%02x data=", (uint)phy_addr, (uint)reg_addr);
		printf("%04X\n", data & 0x0000FFFF);
	}
	return data;
}

int athrs17_phy_reg_write(unsigned char phy_addr, unsigned char reg_addr, unsigned short reg_data)
{
	const char	*devname;
	int rcode = 0;
	...
	if (miiphy_write (devname, phy_addr, reg_addr, reg_data) != 0) {
...
void athrs17_all_port_enable(void)
{
	int i = 0;
	for (i = 0; i < 5; i++) {
		/* Enable auto negotiation and Auto negotiation restart */
		athrs17_phy_reg_write(i, MII_BMCR, BMCR_ANENABLE | BMCR_ANRESTART);
	}
}

void athrs17_all_port_disable(void)
{
	int i = 0;
	for (i = 0; i < 5; i++) {
		/* power down all phy */
		athrs17_phy_reg_write(i, MII_BMCR, BMCR_PDOWN);
	}
}
