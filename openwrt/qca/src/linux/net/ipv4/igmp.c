	...
	int type)
{
	...
	/*  If in_dev->ifa_list == NULL, it means that the IP of device
	 *  has been destroyed(include ip and route info), so the routing 
	 *  procedure would be wrong to anather devices.
	 * */
	if (!in_dev->ifa_list)
		return -1;
