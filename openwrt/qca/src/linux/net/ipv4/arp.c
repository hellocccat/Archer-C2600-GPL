 ...
 */
	...
	if (arp->ar_op == htons(ARPOP_REQUEST) &&
		...
		if (addr_type == RTN_LOCAL) {
			...
			/* goto out; */ /* Maybe need to send GARP, so we let it go ahead, Jason Guo<guodongxian@tp-link.net> */
...
#define GARP_INTERVAL 1 /* 1 second */
		 ...
		 */
		/*printk("[%x]bind mac: %02x-%02x-%02x-%02x-%02x-%02x\n",
				n->nud_state, n->ha[0], n->ha[1], n->ha[2], n->ha[3], n->ha[4], n->ha[5]);*/ 
