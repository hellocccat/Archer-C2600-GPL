...
#define IPQ806X_LED_POWER_BLINK  // this is for POWER_LED blink when boot

#ifdef IPQ806X_LED_POWER_BLINK
static int pwr_led_gpio = 53;
#define LED_POWER_GPIO pwr_led_gpio
#define POWER_BLINK_DELAY 500
#define BOOT_TOTAL_TIME 90000

struct timer_list power_led_blink_timer;
#endif

struct proc_dir_entry *gpio_general_entry = NULL;
...
#ifdef IPQ806X_LED_POWER_BLINK
/*
this funcion will be called every time the timer INT arrives.
*/
static void led_timer_function(unsigned long data)
{
 static int toggle = 0;

  __msm_gpio_set_inout(LED_POWER_GPIO,(toggle%2));
  mb();
  toggle ++;

  if (toggle >= BOOT_TOTAL_TIME/POWER_BLINK_DELAY)  // stop timer after 1 min, temp solution.
  {
   del_timer(&power_led_blink_timer);

   // printk("[LED]del timer\n");
   __msm_gpio_set_inout(LED_POWER_GPIO,1); 
   return ;
  }

  mod_timer(&power_led_blink_timer,jiffies + msecs_to_jiffies(POWER_BLINK_DELAY));
}

static int gpio_general_read (char *page, char **start, off_t off,
               int count, int *eof, void *data)
{
    return count;
}
 
static int gpio_general_write (struct file *file, const char *buf,
                                        unsigned long count, void *data)
{
    if(buf != NULL && buf[0] == '0')
    {
        //printk("input 0\n");
        del_timer(&power_led_blink_timer);
        __msm_gpio_set_inout(LED_POWER_GPIO,0);
    }
    else
    {
        //printk("input 1\n");
...
/*
 create the timer for led blink when system boot.
*/
static int msm_gpio_create_led_blink_timer(void)
{
  if (machine_is_tplink_ad7200())
    pwr_led_gpio = 66;

  gpio_tlmm_config(GPIO_CFG(LED_POWER_GPIO,0,1,GPIO_CFG_PULL_UP,GPIO_CFG_2MA),0);

  //printk("[LED]LED timer creat!!!\n");
  
  init_timer(&power_led_blink_timer);
  power_led_blink_timer.function = led_timer_function;
  power_led_blink_timer.expires= jiffies + msecs_to_jiffies(POWER_BLINK_DELAY);
  power_led_blink_timer.data = 0;

  add_timer(&power_led_blink_timer);

  gpio_general_entry = create_proc_entry("power_led_status", 0644, NULL);
  if (!gpio_general_entry)
    return -ENOENT;
 
  gpio_general_entry->write_proc = gpio_general_write;
  gpio_general_entry->read_proc = gpio_general_read;


  return 0;
}
#endif /*IPQ806X_LED_POWER_BLINK*/
...
#ifdef IPQ806X_LED_POWER_BLINK
    msm_gpio_create_led_blink_timer();
#endif
	return 0;
}
