...
void lease_set_hostname(struct dhcp_lease *lease, char *name, int auth, char *domain, char *config_domain)
{
  ...
  if (name)
      ...
      for (lease_tmp = leases; lease_tmp; lease_tmp = lease_tmp->next)
	{
	  ...
	  /* If the lease hwaddr is not the same with the entry of leases, we don't need to kill
	     the hostname, TP-LINK DHCP Server would like to keep several machine having the same 
	     hostname. Added by xlw, 2015-02-09 */
