...
[AC_REQUIRE([MULE_CHECK_NLS])dnl
	...
	for bfd_ldadd in "" "${LIBINTL}" "-ldl" "-ldl ${LIBINTL}"; do
		...
		AS_IF([test "${with_backtrace_prefix+set}" = set], [
			MULE_PREPEND([CPPFLAGS], [-I$with_backtrace_prefix/include])
			MULE_PREPEND([LIBS], [-L$with_backtrace_prefix/lib])
			MULE_APPEND([LIBS], [-lintl])
		])
		AC_LINK_IFELSE([
			AC_LANG_PROGRAM([[
				#include "ansidecl.h"
			...
			#by zengwei
			AS_IF([test "${with_backtrace_prefix+set}" = set], [
				MULE_PREPEND([BFD_CPPFLAGS], [-L$with_backtrace_prefix/include])
				MULE_PREPEND([BFD_LIBS], [-I$with_backtrace_prefix/lib])
				MULE_APPEND([BFD_LIBS], [-lintl])
			])
			MULE_RESTORE([LIBS])
...
[
	...
	MULE_BACKUP([CPPFLAGS])
	AS_IF([test "${with_backtrace_prefix+set}" = set], [
		MULE_PREPEND([LIBS], [-L$with_backtrace_prefix/lib -lubacktrace -ldl])
		MULE_PREPEND([CPPFLAGS], [-I$with_backtrace_prefix/include])
	], [
		MULE_PREPEND([LIBS], [-ldl])
	])
	AC_LINK_IFELSE([
		AC_LANG_PROGRAM([[
			#include "execinfo.h"
