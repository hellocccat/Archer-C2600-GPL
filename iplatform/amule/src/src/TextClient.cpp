...
enum {
	...
	//zengwei add log level set
	CMD_ID_LOG_SET_DEBUG,
	CMD_ID_SET_RATE_PRINT,
	CMD_ID_SET_DIRECTORY,
	CMD_ID_SET_TCP_PORT,
	CMD_ID_SET_UDP_PORT,
	CMD_ID_GET_PORT,
	CMD_ID_ADD_SERVER,
	CMD_ID_REMOVE_SERVER
...
/*
		...
		case CMD_ID_DOWNLOAD:
			...
			break;
*/
//zengwei add log level set below
		case CMD_ID_LOG_SET_DEBUG:
			if ( ! args.IsEmpty() ) {
				bool debugOn;
				if (args.IsSameAs(wxT("on"), false)) {
					debugOn = true;
				...
				request = new CECPacket(EC_OP_SET_LOG_LEVEL);
				request->AddTag(CECTag(EC_TAG_LOG_SET_DEBUG, (uint8)debugOn));
				request_list.push_back(request);			
			}else{
				Show(_("This command requires an argument. Valid arguments: on or off.\n"));
				return CMD_ERR_INVALID_ARG;
			}
			
			break;
		case CMD_ID_SET_RATE_PRINT:
			if ( ! args.IsEmpty() ) {
				bool debugOn;
				if (args.IsSameAs(wxT("on"), false)) {
					debugOn = true;
				...
				request = new CECPacket(EC_OP_SET_RATE_PRINT);
				request->AddTag(CECTag(EC_TAG_SET_RATE_PRINT, (uint8)debugOn));
				request_list.push_back(request);			
			}else{
				Show(_("This command requires an argument. Valid arguments: on or off.\n"));
				return CMD_ERR_INVALID_ARG;
			...
			break;
		case CMD_ID_SET_DIRECTORY:
			if ( ! args.IsEmpty() ) {
				request = new CECPacket(EC_OP_SET_PREFERENCES);
				CECEmptyTag prefs(EC_TAG_PREFS_DIRECTORIES);
				prefs.AddTag(CECTag(EC_TAG_DIRECTORIES_INCOMING, args.append(wxT("/Incoming")).To8BitData()));
				prefs.AddTag(CECTag(EC_TAG_DIRECTORIES_TEMP, args.append(wxT("/Temp")).To8BitData()));
				request->AddTag(prefs);
				request_list.push_back(request);
			}else{
				Show(_("This command requires an argument. Valid arguments: directory.\n"));
				return CMD_ERR_INVALID_ARG;
			}
			break;
		case CMD_ID_SET_TCP_PORT:
			unsigned long int tcpPort;
			{
				if ( args.ToULong(&tcpPort) ) {
					request = new CECPacket(EC_OP_SET_PREFERENCES);
					CECEmptyTag prefs(EC_TAG_PREFS_CONNECTIONS);
					prefs.AddTag(CECTag(EC_TAG_CONN_TCP_PORT, (uint16)tcpPort));
					request->AddTag(prefs);
					request_list.push_back(request);
				}else{
					Show(_("This command requires valid argument: tcp port.\n"));
					return CMD_ERR_INVALID_ARG;
				}
			}
			break;
		case CMD_ID_SET_UDP_PORT:
			unsigned long int udpPort;
			{
				if ( args.ToULong(&udpPort) ) {
					request = new CECPacket(EC_OP_SET_PREFERENCES);
					CECEmptyTag prefs(EC_TAG_PREFS_CONNECTIONS);
					prefs.AddTag(CECTag(EC_TAG_CONN_UDP_PORT, (uint16)udpPort));
					request->AddTag(prefs);
					request_list.push_back(request);
				}else{
					Show(_("This command requires valid argument: udp port.\n"));
					return CMD_ERR_INVALID_ARG;
				}
			}
			break;
		case CMD_ID_GET_PORT:
			request = new CECPacket(EC_OP_GET_PREFERENCES);
			request->AddTag(CECTag(EC_TAG_SELECT_PREFS, (uint32)EC_PREFS_CONNECTIONS));
			request_list.push_back(request);
			break;
		case CMD_ID_ADD_SERVER:
			if ( ! args.IsEmpty() ) {			
				wxString s_ip = args.Left(args.Find(':'));
				wxString s_port = args.Mid(args.Find(':') + 1);
				uint32 ip = StringIPtoUint32(s_ip);
				long port = StrToULong(s_port);

				if (s_ip && port) {
					request = new CECPacket(EC_OP_SERVER_ADD);
					request->AddTag(CECTag(EC_TAG_SERVER_ADDRESS, args.To8BitData()));
					//request->AddTag(CECTag(EC_TAG_SERVER_NAME, args.To8BitData()));
				}else{
					Show(_("This command has no Valid arguments: ip:port\n"));
					return CMD_ERR_INVALID_ARG;
				}
				request_list.push_back(request);
			}else{
				Show(_("This command requires an argument. Valid arguments: ip:port\n"));
				return CMD_ERR_INVALID_ARG;
			}
			break;
		case CMD_ID_REMOVE_SERVER:
			if ( ! args.IsEmpty() ) {
				wxString s_ip = args.Left(args.Find(':'));
				wxString s_port = args.Mid(args.Find(':') + 1);
				uint32 ip = StringIPtoUint32(s_ip);
				...
				if (s_ip && port) {
					...
					request->AddTag(CECTag(EC_TAG_SERVER, EC_IPv4_t(ip, port)));
				}else{
					Show(_("This command has no Valid arguments: ip:port\n"));
					return CMD_ERR_INVALID_ARG;
				}
			...
			break;
//zengwei add log level set above
		...
		case EC_OP_SET_PREFERENCES:
			{
				...
				const CECTag *connTcpPort = tab->GetTagByName(EC_TAG_CONN_TCP_PORT);
				const CECTag *connUdpPort = tab->GetTagByName(EC_TAG_CONN_UDP_PORT);
				if (connTcpPort && connUdpPort) {
					s << CFormat(_("TCP port: %u, UDP port: %u.\n"))
						% connTcpPort->GetInt() % connUdpPort->GetInt();
				}
			}
			break;
		case EC_OP_STRINGS:
...
void CamulecmdApp::OnInitCommandSet()
{
	...
	tmp->AddCommand(wxT("Kad"), CMD_ID_DISCONNECT_KAD, wxTRANSLATE("Disconnect from Kad only."), wxEmptyString, CMD_PARAM_NEVER);
 	...
 	m_commands.AddCommand(wxT("Addlink"), CMD_ID_ADDLINK, wxTRANSLATE("Add an eD2k or magnet link to core."),
			      wxTRANSLATE("The eD2k link to be added can be:\n*) a file link (ed2k://|file|...), it will be added to the download queue,\n*) a server link (ed2k://|server|...), it will be added to the server list,\n*) or a serverlist link, in which case all servers in the list will be added to the\n   server list.\n\nThe magnet link must contain the eD2k hash and file length.\n"), CMD_PARAM_ALWAYS);
 	m_commands.AddCommand(wxT("Addserver"), CMD_ID_ADD_SERVER, wxTRANSLATE("Add an eD2k server to core."),
			      wxTRANSLATE("The eD2k server to be added must be:\n ip:port\n"), CMD_PARAM_ALWAYS);
 	m_commands.AddCommand(wxT("Removeserver"), CMD_ID_REMOVE_SERVER, wxTRANSLATE("Remove an eD2k server from core."),
			      wxTRANSLATE("The eD2k server to be removed must be:\n ip:port\n"), CMD_PARAM_ALWAYS);
	...
	tmp2->AddCommand(wxT("Down"), CMD_ID_SET_BWLIMIT_DOWN, wxTRANSLATE("Set download bandwidth limit."),
			 wxT("The given value must be in kilobytes/sec.\n"), CMD_PARAM_ALWAYS);
	tmp->AddCommand(wxT("Incomedir"), CMD_ID_SET_DIRECTORY, wxTRANSLATE("Set incoming directory for complete file."),
			       wxEmptyString, CMD_PARAM_ALWAYS);
	tmp->AddCommand(wxT("TcpPort"), CMD_ID_SET_TCP_PORT, wxTRANSLATE("Set tcp port."),
			       wxEmptyString, CMD_PARAM_ALWAYS);
	tmp->AddCommand(wxT("UdpPort"), CMD_ID_SET_UDP_PORT, wxTRANSLATE("Set udp port."),
			       wxEmptyString, CMD_PARAM_ALWAYS);
	...
	tmp->AddCommand(wxT("Port"), CMD_ID_GET_PORT, wxTRANSLATE("Get tcp and udp port."), wxEmptyString, CMD_PARAM_NEVER);
...
// 	tmp->AddCommand(wxT("Shared"), CMD_ID_SHOW_SHARED, wxTRANSLATE("Show shared files list."), wxEmptyString, CMD_PARAM_NEVER);
	...
	m_commands.AddCommand(wxT("Setlogdbg"), CMD_ID_LOG_SET_DEBUG, wxTRANSLATE("Set log debug level."),
				    wxTRANSLATE("The param must be on or off.\n"), CMD_PARAM_ALWAYS);
	m_commands.AddCommand(wxT("Setprintrate"), CMD_ID_SET_RATE_PRINT, wxTRANSLATE("Set print rate."),
				    wxTRANSLATE("The param must be on or off.\n"), CMD_PARAM_ALWAYS);
