...
bool CDownloadQueue::IsHashExistCompletedList(CMD4Hash hash)
{
	wxMutexLocker lock( m_mutex );
...
bool CDownloadQueue::AddED2KLink( const CED2KFileLink* link, uint8 category )
{
	...
	if (GetFileCount() >= MAX_ED2K_DOWNLOAD_FILE_NUM){
		AddLogLineC(CFormat(_("Download file number is up to max(%d) cannot add any more.")) % MAX_ED2K_DOWNLOAD_FILE_NUM);
		return false;			
	}
...
//for debug
void CDownloadQueue::PrintDownloadQueue()
{
	AddLogLineN(CFormat(_("Show DownloadQueue filelistsize %d completed %d completedhash %d localsReq %d")) 
		% GetFileCount()
		% m_completedDownloads.size()
		% m_completedHashList.size()
		% m_localServerReqQueue.size());
