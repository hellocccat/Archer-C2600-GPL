...
//zengwei add below
wxChar HexToDec(char *str, size_t nFirst, size_t nCount, size_t maxLen)
{
	wxChar result = 0;

    // out-of-bounds requests return sensible things
    if ( nCount > (maxLen - nFirst))
    {
        nCount = maxLen - nFirst;
    }

	//nFirst position must < maxLen
    if ( nFirst >= maxLen )
    {
	...
	for ( size_t i = 0; i < nCount; ++i ) {
		result *= 16;
		wxChar cur = (wxChar)wxToupper(str[nFirst+i]);
...
wxString UnescapeHTML(const wxString& str)
{
	...
	for (size_t i = 0; i < len; ++i, ++j) {
		if (buffer[i] == '%' && (len > i + 2)) {
			...
			wxChar unesc = HexToDec(buffer, i + 1, 2, len);
			if (unesc) {
				i += 2;
				buffer[j] = (char)unesc;
			} else {
