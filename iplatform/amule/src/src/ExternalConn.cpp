...
static CECPacket *Get_EC_Response_GetDownloadQueue_Simple(const CECPacket *request)
{	
	CECPacket *response = new CECPacket(EC_OP_DLOAD_QUEUE_SIMPLE);

	for (unsigned int i = 0; i < theApp->downloadqueue->GetFileCount(); i++) {
		CPartFile *cur_file = theApp->downloadqueue->GetFileByIndex(i);
		CECEmptyTag filetag(EC_TAG_PARTFILE);
		uint64_t upRate = 0;
		
		filetag.AddTag(CECTag(EC_TAG_PARTFILE_HASH, cur_file->GetFileHash()));
		filetag.AddTag(CECTag(EC_TAG_PARTFILE_SIZE_FULL, cur_file->GetFileSize()));
		filetag.AddTag(CECTag(EC_TAG_PARTFILE_SIZE_DONE, cur_file->GetCompletedSize()));
		filetag.AddTag(CECTag(EC_TAG_PARTFILE_SOURCE_COUNT, cur_file->GetSourceCount()));
		filetag.AddTag(CECTag(EC_TAG_PARTFILE_SOURCE_COUNT_NOT_CURRENT, cur_file->GetNotCurrentSourcesCount()));
		filetag.AddTag(CECTag(EC_TAG_PARTFILE_SOURCE_COUNT_XFER, cur_file->GetTransferingSrcCount()));
		filetag.AddTag(CECTag(EC_TAG_PARTFILE_SPEED, (uint64_t)(cur_file->GetKBpsDown()*1024)));
		...
		filetag.AddTag(CECTag(EC_TAG_PARTFILE_UP_SPEED, upRate));
...
*/
			default:
				...
				response->AddTag(CECTag(EC_TAG_STRING, 
					CFormat(wxString(wxTRANSLATE("OpCode %d processing error!"))) %request->GetOpCode()));
				break;
		}
	}
	if (!response) {
...
#if 1 //zengwei add auto connect server
		if (thePrefs::DoAutoConnect() && !theApp->IsConnectedED2K()) {
			theApp->serverconnect->ConnectToAnyServer();
		}
#endif //zengwei add auto connect server
...
/*
		...
		case EC_OP_GET_SHARED_FILES:
			...
			break;
*/
		case EC_OP_GET_DLOAD_QUEUE_SIMPLE:
			response = Get_EC_Response_GetDownloadQueue_Simple(request);
			break;
...
/*
		...
		case EC_OP_ADDDEBUGLOGLINE:
			...
			break;
*/
//zengwei add log below
		case EC_OP_SET_LOG_LEVEL:
			if (request->GetFirstTagSafe()->GetInt()){
				AddLogLineN(CFormat(_("Set debug log on")));
				theLogger.SetLogDebug(true);
			}else {
				AddLogLineN(CFormat(_("Set debug log off")));
				theLogger.SetLogDebug(false);
			}
			response = new CECPacket(EC_OP_NOOP);
			break;
		case EC_OP_SET_RATE_PRINT:
			if (request->GetFirstTagSafe()->GetInt()){
				AddLogLineN(CFormat(_("Set rate print log on")));
				theApp->m_enableRatePrint = true;
			}else {
				AddLogLineN(CFormat(_("Set rate print log off")));
				theApp->m_enableRatePrint = false;
			}
			response = new CECPacket(EC_OP_NOOP);
			break;
//zengwei add log above
		case EC_OP_GET_LOG:
