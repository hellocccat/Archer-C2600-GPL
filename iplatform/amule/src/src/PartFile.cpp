...
void CPartFile::PartFileHashFinished(CKnownFile* result)
{
	...
	if (theApp->sharedfiles->GetFilesMap() >= MAX_ED2K_SHARED_FULL_FILE_NUM){
		AddLogLineC(CFormat(_("Shared full file number is up to max(%d) cannot add any more.")) % theApp->sharedfiles->GetFilesMap());
		return;			
	}
#endif //zengwei add restrict share number

	AddLogLineC(CFormat(_("Shared file add %s.")) % GetFileName());
...
void CPartFile::CompleteFile(bool bIsHashingDone)
{
	...
	if (!bIsHashingDone) {
		...
		//printf("mod hash CompleteFile no hash\n");
...
{	
	if (errorOccured) {
		...
		if (kfile != NULL){
			AddLogLineC(CFormat(_("Cancel shared part file %s when complete downloading.")) % kfile->GetFileName());
			theApp->sharedfiles->RemoveFile(this);
		}
		
#ifndef __WXMSW__ //zengwei add
#ifdef ENABLE_UBUS_RPC
		theApp->downloadqueue->AddCompletedHash(this->GetFileHash());
...
{	
	...
	if (errorOccured) {
		...
		//check the completed file whether in the sharedfilelist already. 
		CKnownFile *kfile = theApp->sharedfiles->GetFileByID(this->GetFileHash());
		AddDebugLogLineN(logPartFile, CFormat(wxT("CompleteFileEnded = %d")) % (int)kfile);

		if ((theApp->sharedfiles->GetFilesMap() < MAX_ED2K_SHARED_FULL_FILE_NUM) || 
			((kfile != NULL) && (theApp->sharedfiles->GetFilesMap() == MAX_ED2K_SHARED_FULL_FILE_NUM))){
			theApp->sharedfiles->SafeAddKFile(this);
			theApp->knownfiles->SafeAddKFile(this);
			AddLogLineC(CFormat(_("Shared file add %s now shared files %d.")) % GetFileName() % theApp->sharedfiles->GetFilesMap());
		}else{
			AddLogLineC(CFormat(_("Shared full file number is up to max(%d) cannot add complete part.")) % theApp->sharedfiles->GetFilesMap());
			shareFlag = false;
			//shared before zw		
			theApp->sharedfiles->RemoveFile(this);
		}
					...
					if (theApp->IsRunning()) { // may be called during shutdown!
						if (GetHashCount() == GetED2KPartHashCount() && !m_hashsetneeded) {
							...
							if (theApp->sharedfiles->GetFilesMap() < MAX_ED2K_SHARED_FULL_FILE_NUM){
								...
								AddLogLineC(CFormat(_("Shared full file number is up to max(%d) cannot add part more.")) % theApp->sharedfiles->GetFilesMap());
							}
#endif //zengwei add restrict share number							
							//theApp->sharedfiles->SafeAddKFile(this);
						}
			...
			if (status == PS_EMPTY && theApp->IsRunning()) {
				if (GetHashCount() == GetED2KPartHashCount() && !m_hashsetneeded) {
					...
					if (theApp->sharedfiles->GetFilesMap() < MAX_ED2K_SHARED_FULL_FILE_NUM){
						...
						AddLogLineC(CFormat(_("Shared full file number is up to max(%d) cannot add recov part.")) % theApp->sharedfiles->GetFilesMap());
					}
#endif //zengwei add restrict share number							
					//theApp->sharedfiles->SafeAddKFile(this);
				}
...
//for debug
void CPartFile::PrintPartFile()
{
	AddLogLineN(CFormat(_("----deadSources %d reqblocks %d BuffData %d SrcList %d A4AFsrclist %d")) 
		% m_deadSources.GetDeadSourcesCount()
		% m_requestedblocks_list.size()
		% m_BufferedData_list.size()
		% m_SrcList.size()
		% m_A4AFsrclist.size());
}
