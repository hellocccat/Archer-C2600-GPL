...
void CSharedFileList::PrintSharedFileList()
{
	
	AddLogLineN(CFormat(_("Show SharedFileList filemapsize %d")) % m_Files_map.size());

	CKnownFileMap::iterator it = m_Files_map.begin();
	for (; it != m_Files_map.end(); ++it) {
		const CKnownFile* file = it->second;
		AddLogLineN(CFormat(_("----filename(%s) partfile?(%d) uploadlistsize(%d)")) 
			%file->GetFileName() % file->IsPartFile() % file->GetQueuedCount());		
	}
}
