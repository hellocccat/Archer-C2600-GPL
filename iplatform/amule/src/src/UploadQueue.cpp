...
void CUploadQueue::AddClientToQueue(CUpDownClient* client)
{
	...
	if (theApp->serverconnect->IsConnected() && theApp->serverconnect->IsLowID() && !theApp->serverconnect->IsLocalServer(client->GetServerIP(),client->GetServerPort()) && client->GetDownloadState() == DS_NONE && m_waitinglist.size() > 50) {
...
//				if (cur_client->GetFriendSlot() || cur_client->GetUploadFile()->GetUpPriority() == PR_POWERSHARE) {
			if (cur_client->GetUploadFile()->GetUpPriority() == PR_POWERSHARE) {			
				vips++;
			}
		}
...
//for debug
void CUploadQueue::PrintUploadQueue()
{		
	AddLogLineN(CFormat(wxT("Show uploadwaiting list")));

	CClientRefList::iterator it = m_waitinglist.begin();
	for (it = m_waitinglist.begin(); it != m_waitinglist.end(); it++) {
		CUpDownClient* c = it->GetClient();
		AddLogLineN(CFormat(wxT("----%4d %7d  %s %5d  %s"))
			% c->GetUploadQueueWaitingPosition()
			% c->GetScore()
			% (c->HasLowID() ? (c->IsConnected() ? wxT("LoCon") : wxT("LowId")) : wxT("High "))
			% c->ECID()
	...
	AddLogLineN(CFormat(wxT("Show uploading list")));

	it = m_uploadinglist.begin();
	for (it = m_uploadinglist.begin(); it != m_uploadinglist.end(); it++) {
		CUpDownClient* c = it->GetClient();
		AddLogLineN(CFormat(wxT("----%4d %7d  %s %5d  %s"))
			% c->GetUploadQueueWaitingPosition()
			% c->GetScore()
