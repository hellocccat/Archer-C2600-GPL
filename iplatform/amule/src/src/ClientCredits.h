...
public:
	...
	uint32	GetCleanTime() const { return m_dwCleanTime; }		//zengwei add
	void	SetCleanTime(uint32 val) { m_dwCleanTime = val; }	//zengwei add
	uint64	GetCleanTimeDownloaded() const { return m_cleanTimeDownloaded; }	//zengwei add
	void	SetCleanTimeDownloaded(uint64 val) { m_cleanTimeDownloaded = val; }	//zengwei add
...
private:
	...
	uint32			m_dwIdentIP;
	uint32			m_dwCleanTime;				//zengwei add for clean the credits
	uint64			m_cleanTimeDownloaded;		//zengwei add for clean the credits
	//uint32			m_dwWaitTime;
