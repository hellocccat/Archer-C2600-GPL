...
DEFINE_LOCAL_EVENT_TYPE(MULE_EVT_LOGFILESTATUS) //zengwei add
...
// handle Debug messages
	if (type != logStandard) {
		//zengwei mod log level set
		//if (!critical && !IsEnabled(type)) {
		if (!critical && !IsEnabled(type) && !GetLogSetDebug()) {
			return;
		}
		int index = (int)type;
...
//zengwei add
void CLogger::CheckLogFileStatus(const wxString& logfile)
{
	CPath logfilePath = CPath(logfile);
	//file not exist or up to max size or closed event
	if (!logfilePath.FileExists() || 
		((sint64)(logfilePath.GetFileSize())>AMULE_LOGFILE_SIZE) ||
		!IsLogfileOpened()) {

		CLogFileStatusEvent Event(logfile);
...
//zengwei add
void CLogger::OnLogFileStatusEvent(class CLogFileStatusEvent& evt)
{			

	CPath logfilePath = CPath(evt.GetLogFile());
	
//		// Make a backup of the log file
//		if ((sint64)(logfilePath.GetFileSize()) > AMULE_LOGFILE_SIZE &&
//			logfilePath.FileExists()){
//			CPath::BackupFile(logfilePath, wxT(".1"));
//		}
	...
	if (logfilePath.FileExists()){
		CPath::BackupFile(logfilePath, wxT(".bak"));
	}
	
	if (!OpenLogfile(logfilePath.GetRaw())) {
...
BEGIN_EVENT_TABLE(CLogger, wxEvtHandler)
	...
	EVT_MULE_LOGFILESTATUS(CLogger::OnLogFileStatusEvent)
END_EVENT_TABLE()
