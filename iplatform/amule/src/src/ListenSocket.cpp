...
//for debug
void CListenSocket::PrintListenSocket()
{
	
	AddLogLineN(CFormat(_("Show ListenSocket socket_list %d")) 
			% socket_list.size());

	for (SocketSet::iterator it = socket_list.begin(); it != socket_list.end(); ) {
		CClientTCPSocket* cur_socket = *it++;
		int a = cur_socket ? cur_socket->GetControlQueue() : 0;
		int b = cur_socket ? cur_socket->GetStandardQueue() : 0;
		
		AddLogLineN(CFormat(_("----ControlQueue_s %d StandardQueue_s %d")) % a % b);
	}
