...
void CClientList::PrintClientList()
{	
	AddLogLineN(CFormat(_("Show ClientList hashList %d iplist %d clientList %d bannedList %d" 
			"trackedClientsList %d deadSources %d")) 
			% m_hashList.size()
			% m_ipList.size()
			% m_clientList.size()
			% m_bannedList.size()
			% m_trackedClientsList.size()
			% m_deadSources.GetDeadSourcesCount());

	AddLogLineN(CFormat(_("Show client 1-A4AFList_s 2-WaitingPack_s 3-BlockReq_s 4-Donelist_s 5-Plist_s 6-Downlist_s 7-SourceFrom 8-UploadState 9-DownloadState 10-control_queue 11-standard_queue")));

	for (IDMap::iterator current_it = m_clientList.begin(); current_it != m_clientList.end(); current_it++) {
		CUpDownClient* pCurClient = current_it->second.GetClient();

		int a = pCurClient->m_socket ? pCurClient->m_socket->GetControlQueue() : 0;
		int b = pCurClient->m_socket ? pCurClient->m_socket->GetStandardQueue() : 0;
		if (pCurClient->m_A4AF_list.size()
			|| pCurClient->m_WaitingPackets_list.size()
			|| pCurClient->m_BlockRequests_queue.size()
			|| pCurClient->m_DoneBlocks_list.size()
			|| pCurClient->m_PendingBlocks_list.size()
			|| pCurClient->m_DownloadBlocks_list.size()
			|| a || b){
			AddLogLineN(CFormat(_("----1-%d\t 2-%d\t 3-%d\t 4-%d\t 5-%d\t 6-%d\t 7-%d\t 8-%d\t 9-%d\t 10-%d\t 11-%d\t ")) 
				% pCurClient->m_A4AF_list.size()
				% pCurClient->m_WaitingPackets_list.size()
				% pCurClient->m_BlockRequests_queue.size()
				% pCurClient->m_DoneBlocks_list.size()
				% pCurClient->m_PendingBlocks_list.size()
				% pCurClient->m_DownloadBlocks_list.size()
				% (int)pCurClient->GetSourceFrom()
