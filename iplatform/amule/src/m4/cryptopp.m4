...
m4_define([MIN_CRYPTO_VERSION], [m4_ifval([$1], [$1], [5.1])])dnl
	...
	AC_ARG_WITH([crypto-prefix],
		[AS_HELP_STRING([--with-crypto-prefix=PREFIX], [prefix where crypto++ or libtom is installed])])
	...
	AS_IF([test -n "$with_crypto_prefix"], [
		...
		MULE_IF([test -f $CRYPTOPP_PREFIX/$cryptopp_file_with_version], [
			...
			CRYPTOPP_INCLUDE_PREFIX="$CRYPTOPP_LIB_NAME"
			cryptopp_libdir="$CRYPTOPP_PREFIX/lib"
		], [test -f $CRYPTOPP_PREFIX/include/tomcrypt/tomcrypt.h], [
			#use tomcrypt to replace cryptopp by zengwei
			cryptopp_file_with_version="tomcrypt.h"
			CRYPTOPP_STYLE="tom_installed"
			CRYPTOPP_LIB_NAME="tomcrypt"
			cryptopp_includedir="$CRYPTOPP_PREFIX/include"
			CRYPTOPP_INCLUDE_PREFIX="$CRYPTOPP_LIB_NAME"
			cryptopp_libdir="$CRYPTOPP_PREFIX/lib"
			CRYPTOPP_LIB_DEP_NAME="tommath"
		])
	], [
	...
	AS_IF([test $CRYPTOPP_STYLE = "unknown"], [result=no; resultstr=""], 
		[test $CRYPTOPP_STYLE = "tom_installed"], [
		# Find out the crypto++ version and check against the minimum required by zengwei
		cryptopp_header_path="${cryptopp_includedir+$cryptopp_includedir/}$CRYPTOPP_INCLUDE_PREFIX/$cryptopp_file_with_version"
		CRYPTOPP_VERSION_STRING=`$EGREP "SCRYPT" $cryptopp_header_path | sed -e ['s/[^0-9]*\([0-9.]*\).*/\1/']`
		CRYPTOPP_VERSION_NUMBER=`echo $CRYPTOPP_VERSION_STRING | $AWK 'BEGIN { FS = "."; } { printf "%d", ([$]1 * 1000 + [$]2) * 1000 + [$]3;}'`
		minvers=`echo 1.17 | $AWK 'BEGIN { FS = "."; } { printf "%d", ([$]1 * 1000 + [$]2) * 1000 + [$]3;}'`
		...
		AS_IF([test -n "$CRYPTOPP_VERSION_NUMBER" && test "$CRYPTOPP_VERSION_NUMBER" -ge $minvers], [
			...
			AS_IF([test -n "$cryptopp_includedir"], [
				CRYPTOPP_CPPFLAGS="-I$cryptopp_includedir/$CRYPTOPP_INCLUDE_PREFIX"
			], [
				CRYPTOPP_CPPFLAGS=
			])
			AS_IF([test -n "$cryptopp_libdir"], [
				...
				AS_IF([test -n "$CRYPTOPP_LIB_DEP_NAME"], [
					CRYPTOPP_LIBS="$CRYPTOPP_LIBS -l$CRYPTOPP_LIB_DEP_NAME"
				])
			], [
				CRYPTOPP_LDFLAGS=
				CRYPTOPP_LIBS="${CRYPTOPP_INCLUDE_PREFIX}/lib${CRYPTOPP_LIB_NAME}.a"
			])
			AH_TEMPLATE([CRYPTOPP_INCLUDE_PREFIX], [Define this to the include prefix of tomcrypt])
			AC_DEFINE_UNQUOTED([CRYPTOPP_INCLUDE_PREFIX], $CRYPTOPP_INCLUDE_PREFIX)
			CRYPTOPP_CPPFLAGS="$CRYPTOPP_CPPFLAGS -DLTM_DESC -DLTC_SOURCE -DENABLE_TOMCRYPT"
		], [
			result=no
		...
		AS_IF([test -n "$CRYPTOPP_VERSION_NUMBER" && test "$CRYPTOPP_VERSION_NUMBER" -ge $minvers], [
			...
			AC_DEFINE_UNQUOTED([CRYPTOPP_INCLUDE_PREFIX], $CRYPTOPP_INCLUDE_PREFIX)
			#by zengwei
			AS_IF([test $CRYPTOPP_VERSION_NUMBER -ge "5005000"], [
