...
#define	MD4_NEW_DIGEST_STRING_LENGTH	(MD4_NEW_DIGEST_LENGTH * 2 + 1)
...
static u8 PADDING[MD4_NEW_BLOCK_LENGTH] = 
{
 ...
 */
static void MD4_NEW_Init(MD4_NEW_CTX *ctx)
{
	ctx->count = 0;
 ...
 */
static void
MD4_NEW_Transform(u32 state[4], const u8 block[MD4_NEW_BLOCK_LENGTH])
{
	u32 a, b, c, d, in[MD4_NEW_BLOCK_LENGTH / 4];

#if BYTE_ORDER == LITTLE_ENDIAN
	memcpy(in, block, sizeof(in));
#else
	for (a = 0; a < MD4_NEW_BLOCK_LENGTH / 4; a++) 
	{
		in[a] = (u32)(
			(u32)(block[a * 4 + 0]) |
			(u32)(block[a * 4 + 1]) <<	8 |
 ...
 * of bytes.
 */
static void MD4_NEW_Update(MD4_NEW_CTX *ctx, const unsigned char *input, size_t len)
{
	size_t have, need;

	/* Check how many bytes we already have and how many more we need. */
	have = (size_t)((ctx->count >> 3) & (MD4_NEW_BLOCK_LENGTH - 1));
	need = MD4_NEW_BLOCK_LENGTH - have;
	...
	if (len >= need)
	{
		if (have != 0) 
		{
			...
			MD4_NEW_Transform(ctx->state, ctx->buffer);
			input += need;
			len -= need;
			have = 0;
		}
		...
		while (len >= MD4_NEW_BLOCK_LENGTH) 
		{
			MD4_NEW_Transform(ctx->state, input);
			input += MD4_NEW_BLOCK_LENGTH;
			len -= MD4_NEW_BLOCK_LENGTH;
		}
	}
 ...
 */
static void MD4_NEW_Pad(MD4_NEW_CTX *ctx)
{
	u8 count[8];
	size_t padlen;
	...
	padlen = MD4_NEW_BLOCK_LENGTH -
		((ctx->count >> 3) & (MD4_NEW_BLOCK_LENGTH - 1));
	if (padlen < 1 + 8)
	{
		padlen += MD4_NEW_BLOCK_LENGTH;
	}
	MD4_NEW_Update(ctx, PADDING, padlen - 8);		/* padlen - 8 <= 64 */
	MD4_NEW_Update(ctx, count, 8);
	return;
}
 ...
 */
static void MD4_NEW_Final(unsigned char digest[MD4_NEW_DIGEST_LENGTH], MD4_NEW_CTX *ctx)
{
	int i;
...
void md4_vector(size_t num_elem, const u8 *addr[], const size_t *len, u8 *mac)
{
	...
	{
		MD4_NEW_Update(&ctx, addr[i], len[i]);
	}
	MD4_NEW_Final(mac, &ctx);
	return;
}
