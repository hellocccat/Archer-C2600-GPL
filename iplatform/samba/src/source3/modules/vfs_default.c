...
#define FAT_MAX_FILE_SIZE		(0x00000000ffffffff)	//4//4GB
#define HFS_SUPER_MAGIC			(0x4244)
#define HFS_MAX_FILE_SIZE		(0x000000007fffffff)	//2//2GB
#define HFSPLUS_SUPER_MAGIC		(0x482b)
#define HFSPLUS_MAX_FILE_SIZE	(0x0fffffffffffffff)	//8EB
...
static int vfswrap_ftruncate(vfs_handle_struct *handle, files_struct *fsp, SMB_OFF_T len)
{
	...
	if (fs_buf.f_type == FAT_SUPER_MAGIC 
		|| fs_buf.f_type == HFS_SUPER_MAGIC 
		|| fs_buf.f_type == HFSPLUS_SUPER_MAGIC) //FAT, HFS, HFS+	modified by zgs
	{ 
		...
		if ((fs_buf.f_type == FAT_SUPER_MAGIC && len > FAT_MAX_FILE_SIZE) ||
			(fs_buf.f_type == HFS_SUPER_MAGIC && len > HFS_MAX_FILE_SIZE) ||
			(fs_buf.f_type == HFSPLUS_SUPER_MAGIC && len > HFSPLUS_MAX_FILE_SIZE))
		{
			errno = EFBIG; 
			goto done;
		}  
		...
		space_avail = (double)(fs_buf.f_bfree * fs_buf.f_bsize / 1024);	//modified by zgs
